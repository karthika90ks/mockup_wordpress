...
//echo"<pre>", print_r( $_POST, true ), "</pre>"; die;        
        ...
        //Removed template option in v 3.1.0. Keep data for a while.
        abcfl_mbsave_save_txt($postID, 'spgCntrStyle', '_spgCntrStyle');
        //delete_post_meta( $postID, '_spgCntrStyle' );
        //abcfl_mbsave_save_txt($postID, 'spgCntrTM', '_spgCntrTM'); Not implemented
        ...
        // ISOTOPE OK
        abcfl_mbsave_save_cbo( $postID,'imgsLoaded', '_imgsLoaded', 1);
        ...
        for ( $i = 1; $i <= 40; $i++ ) {
            $this->save_input_field( $postID, 'F' . $i );
        }
    }
    ...
    private function update_menu_order( $tplateID ) {
        ...
        //$sortTypeNew = ( isset( $_POST['sortType'] ) ? esc_attr( $_POST['sortType'] ) : 'M' );

        //Manual or Sort Text Field.
        $sortTypeNew = ( isset( $_POST['sortType'] ) ? esc_attr( $_POST['sortType'] ) : 'T' );

        //abcfl_mbsave_save_cbo($tplateID, 'sortType', '_sortType', 'T');
        ...
        //if( $sortTypeOld == 'T' ){ return; }

        //$sortTypeNew = ( isset( $_POST['sortType'] ) ? esc_attr( $_POST['sortType'] ) : 'M' );
        //No need to update sort order. It's already updated by: abcfsl_ajax_update_list_order
        //if( $sortTypeNew == 'M' ){ return; }

        //tplateID = parentID
        //if( $sortTypeOld != $sortTypeNew ) { abcfsl_dba_update_menu_order_sort_txt( $tplateID); }
...
//        abcfl_mbsave_save_txt($postID, 'pImgUrlS', '_pImgUrlS');
    ...
    private function save_input_field( $postID, $F ) {
        ...
        //-- DATE --------------------------
        abcfl_mbsave_save_txt($postID, 'dteDisplayLbl_' . $F, '_dteDisplayLbl_' . $F);
    ...
    private function delete_tplate_field( $postID, $F ) {
        ...
        //-- DATE -------------------------
        delete_post_meta( $postID, '_dteDisplayLbl_' . $F );  
