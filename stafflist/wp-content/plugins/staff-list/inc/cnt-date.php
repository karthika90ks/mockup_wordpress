...
function abcfsl_cnt_date_formated( $dteYMD, $dtFormat ){
    ...
    switch ( $dtFormat ) {
        ...
        case 'DMY-E':
            ...
            break;            
        case 'MD':
            $out = abcfsl_cnt_date_txt( $dteYMD, 'MD' );
            break;
        case 'DM':
            $out = abcfsl_cnt_date_txt( $dteYMD, 'DM' );
            break;
        case 'DM,Y':
            $out = abcfsl_cnt_date_txt( $dteYMD, 'DM,Y' );
            break;
        case 'DMY':
            $out = abcfsl_cnt_date_txt( $dteYMD, 'DMY' );
            break;                         
        default:
            break;
    }
...
function abcfsl_cnt_date_txt( $dteYMD, $dtFormat ){

    //$dteYMD = '2009-02-17';
    $mName = abcfsl_cnt_date_month_name( $dteYMD );
    $dName = substr( $dteYMD, 8 );
    $yName = substr( $dteYMD, 0, 4 );
    $out = '';
...
function abcfsl_cnt_date_month_name( $dteYMD ){
...
function abcfsl_cnt_date_staff_ids_DATEPLUS( $parentID, $scodeCat, $datePlus ){

    if ( abcfsl_util_is_active_bday() ){     
        $postIDs = abcfslub_date_range_staff_ids_DATEPLUS( $parentID, $scodeCat, $datePlus );
        return $postIDs;
    }

    echo"<pre>", print_r( abcfsl_txt(1), true ), "</pre>";  die;    
}