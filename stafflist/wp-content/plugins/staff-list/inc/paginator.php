<?php
//-- PG Paginator START ---------------------------------------
//Return post IDs and count. IDs are filtered and sorted. 
//Used for all layouts and all filter options.
//Called from cnt-... functions.
function abcfsl_paginator_post_ids( $par, $menu, $filters ){

    $args = wp_parse_args( $par, abcfsl_paginator_post_ids_defaults() );

    //echo"<pre>", print_r( $args, true ), "</pre>";   

    $staffID = $args['staffID'];
    $top = $args['top'];
    $pgnationPgQty = $args['pgnationPgQty'];
    $postIDs = array();
    ...
    $postIDs = abcfsl_db_staff_member_ids( $args, $menu, $filters );
    ...
    $out['postIDs'] = abcfsl_util_page_post_ids( $postIDs, $args['pageNo'], $pgnationPgQty );
    return $out;
}
